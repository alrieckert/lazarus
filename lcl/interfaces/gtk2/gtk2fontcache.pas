...
uses
  Classes, SysUtils, FPCAdds, LCLProc, LCLType, AvgLvlTree, gdk2, gtkdef,
  LCLResCache;
...
type
  ...
  public
    ...
    function FindGDKFont(ThePangoLayout: PPangoLayout): TGdkFontCacheItem;
    function FindGDKFontDesc(const LogFont: TLogFont;
                           const LongFontName: string): TGdkFontCacheDescriptor;
    function FindADescriptor(ThePangoLayout: PPangoLayout): TGdkFontCacheDescriptor;
    function Add(ThePangoLayout: PPangoLayout; const LogFont: TLogFont;
                 const LongFontName: string): TGdkFontCacheDescriptor;
    procedure Reference(ThePangoLayout: PPangoLayout);
    procedure Unreference(ThePangoLayout: PPangoLayout);
...
function CompareGdkFontWithResItem(Font: PPangoLayout;
  Item: TGdkFontCacheItem): integer;
begin
  Result := ComparePointers(Font, Item.Layout);
end;
...
function TGdkFontCache.FindGDKFont(ThePangoLayout: PPangoLayout): TGdkFontCacheItem;
var
  ANode: TAvgLvlTreeNode;
begin
  ANode:=FItems.Findkey(ThePangoLayout,TListSortCompare(@CompareGdkFontWithResItem));
  if ANode<>nil then
...
function TGdkFontCache.FindADescriptor(ThePangoLayout: PPangoLayout
  ): TGdkFontCacheDescriptor;
var
  Item: TGdkFontCacheItem;
begin
  Item:=FindGDKFont(ThePangoLayout);
  if Item=nil then
    Result:=nil
  else
...
function TGdkFontCache.Add(ThePangoLayout: PPangoLayout; const LogFont: TLogFont;
  const LongFontName: string): TGdkFontCacheDescriptor;
var
  Item: TGdkFontCacheItem;
begin
  ...
  // find cache item with ThePangoLayout
  Item:=FindGDKFont(ThePangoLayout);
  if Item=nil then begin
    // create new item
    Item:=TGdkFontCacheItem.Create(Self,0);
    Item.GdkFont:=ThePangoLayout;
    gdk_font_ref(ThePangoLayout);
    FItems.Add(Item);
  end;
...
procedure TGdkFontCache.Reference(ThePangoLayout: PPangoLayout);
var
  Item: TGdkFontCacheItem;
begin
  Item:=FindGDKFont(ThePangoLayout);
...
procedure TGdkFontCache.Unreference(ThePangoLayout: PPangoLayout);
var
  Item: TGdkFontCacheItem;
begin
  Item:=FindGDKFont(ThePangoLayout);
  if Item=nil then
    gdk_font_unref(ThePangoLayout)
  else
