...
procedure SetAdditionalProjectSrcPathToCodeToolBoss(const SrcPath: string);
...
begin
  if SrcPath<>'' then begin
    DefTempl:=TDefineTemplate.Create('ProjectSrcPathAdd',
      'Project Source Path addition',ExternalMacroStart+'SRCPATH',
      ConvertTransferMacrosToExternalMacros(SrcPath)+';'
        +'$('+ExternalMacroStart+'SRCPATH)',
        da_DefineAll);
    DefTempl.Flags:=[dtfAutoGenerated,dtfProjectSpecific];
    CodeToolBoss.DefineTree.ReplaceSameName(DefTempl);
  end else begin
    CodeToolBoss.DefineTree.RemoveDefineTemplateByName('ProjectSrcPathAdd');
  end;
end;
