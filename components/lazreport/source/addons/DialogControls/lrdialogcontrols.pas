...
uses
  ...
  LMessages, LCLType, LCLIntf, Buttons, EditBtn;
...
type
  ...
  TlrRadioButton = class(TlrVisualControl)
  private
    function GetChecked: boolean;
    procedure SetChecked(AValue: boolean);
  public
...
begin
  ...
  BaseName := 'lrDateEdit';
  FControl:=TDateEdit.Create(OwnerForm);
...
begin
  inherited Create(AOwnerPage);
  BaseName := 'lrListBox';
  FControl:=TListBox.Create(OwnerForm);
  FControl.Parent:=OwnerForm;
  FControl.AutoSize:=true;
end;
...
begin
  inherited Create(AOwnerPage);
  BaseName := 'lrMemo';
  FControl:=TMemo.Create(OwnerForm);
  FControl.Parent:=OwnerForm;
...
function TlrRadioButton.GetChecked: boolean;
begin
  Result:=TRadioButton(FControl).Checked;
end;

procedure TlrRadioButton.SetChecked(AValue: boolean);
begin
  TRadioButton(FControl).Checked:=AValue;
end;
...
begin
  ...
  FControl:=TRadioButton.Create(OwnerForm);
  FControl.Parent:=OwnerForm;
  FControl.AutoSize:=true;
end;

procedure TlrRadioButton.LoadFromXML(XML: TLrXMLConfig; const Path: String);
begin
  inherited LoadFromXML(XML, Path);
  Checked:=XML.GetValue(Path+'Checked/Value'{%H-}, false);
end;

procedure TlrRadioButton.SaveToXML(XML: TLrXMLConfig; const Path: String);
begin
  inherited SaveToXML(XML, Path);
...
begin
  inherited Create(AOwnerPage);
  BaseName := 'lrComboBox';
  FControl:=TComboBox.Create(OwnerForm);
  FControl.Parent:=OwnerForm;
  FControl.AutoSize:=true;
end;
...
begin
  TCheckBox(FControl).Checked:=AValue;
end;
...
begin
  inherited Create(AOwnerPage);
  BaseName := 'lrCheckBox';
  FControl:=TCheckBox.Create(OwnerForm);
  FControl.Parent:=OwnerForm;
  FControl.AutoSize:=true;
//  CheckBox1: TCheckBox;
end;
...
begin
  inherited Create(AOwnerPage);
  BaseName := 'lrButton';
  FControl:=TBitBtn.Create(OwnerForm);
  FControl.Parent:=OwnerForm;
  FControl.AutoSize:=true;
end;
...
begin
  inherited Create(AOwnerPage);
  BaseName := 'lrEdit';
  FControl:=TEdit.Create(OwnerForm);
  FControl.Parent:=OwnerForm;
...
begin
  inherited Create(AOwnerPage);
  BaseName := 'lrLabel';
  FControl:=TLabel.Create(OwnerForm);
  FControl.Parent:=OwnerForm;
end;
...
begin
  ...
  begin
    ...
    BMP.Canvas.FillRect(Rect(0, 0, dx + 1, dy + 1));
    if Caption <> '' then
      BMP.Canvas.TextRect(Rect(0, 0, dx + 1, dy + 1), 0,0, Caption)
    else
      BMP.Canvas.TextRect(Rect(0, 0, dx + 1, dy + 1), 0,0, Name)
  end
  else
  begin
    FControl.Left := 0;
    ...
    FControl.Width := DX;
    FControl.Height := DY;
    Control.Perform(LM_ERASEBKGND, MemDC, MemDC);
    Control.Perform(LM_PAINT, MemDC, MemDC);
  end;
