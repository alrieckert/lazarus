  ...
  case fRange of
    rsUnknown, rsCommand: begin
                 ...
                 if fTokenID <> tkEmbedSQL then begin
                   ...
                   brace := 1; firstBrace := true;
                   inc( Run, fStringLen );
                 end;
               end;
    rsSQL: begin
             fTokenID := tkEmbedSQL;
             while (FLine[Run] <> #0) and (brace<>0) do begin
               case FLine[Run] of
                 '(': if not firstBrace then inc(brace)
                      else firstBrace := false;
                 ')': dec(brace);
               end;
               inc(Run);
             end;
             if brace=0 then fRange := rsUnknown;
           end;
    rsHTML: begin
              fTokenID := tkEmbedSQL;
              while (FLine[Run] <> #0) and (brace<>0) do begin
