...
Known Issues:  - to be implemented: limit font selection for text body to
                 fixed width fonts
               - to be implemented: word-wrap for normal text
...
type
  ...
  TSynPrintFilter = (pfAll, pfEven, pfOdd);                                     //rs 2000-07-14
  ...
  protected
    ...
    procedure PrintHeaderFooterLines(ACanvas: TCanvas; Footer: boolean);
  ...
  public
    ...
    property Filter: TSynPrintFilter read fFilter write fFilter;                //rs 2000-07-14
...
procedure TSynPrintout.PrintHeaderFooterLines(ACanvas: TCanvas; Footer: boolean);
var
  HFLines: TStrings;
  i, Count: integer;
...
var
  PagePrinted: boolean;                                                         //rs 2000-07-14
begin
  EnsurePagination;
  ...
  if (FirstPage <= LastPage) and not DoCancelPrint(psBegin, 0) then begin
    ...
    try
      ...
      PagePrinted := FALSE;                                                     //rs 2000-07-14
      while TRUE do begin
        if DoCancelPrint(psNewPage, FirstPage) then
          break;
        try
          PagePrinted := ShouldPrint(FirstPage);                                //rs 2000-07-14
          if PagePrinted then                                                   //rs 2000-07-14
            PrintPage(FirstPage);
        except
          break;
        end;
...
begin
  ...
  if ForceWrap then
    ...
    Wrap := 0;
  // this implementation wraps at spaces only, if there is none it will just
  // return MaxLength
