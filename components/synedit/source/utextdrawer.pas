...
//ADDED FOR LAZARUS
  {$IFDEF MWE_FPC}
    function InternalCreateFont(fStyle: TFontStyles): HFONT; virtual;
  {$ELSE}
...
//ADDED FOR LAZARUS
  {$IFDEF MWE_FPC}
    constructor Create(CalcExtentBaseStyle: TFontStyles; _BaseFont: TFont); virtual;
  {$ELSE}
...
//ADDED FOR LAZARUS
  {$IFDEF MWE_FPC}
function TheFontStock.InternalCreateFont(fStyle: TFontStyles): HFONT;
  {$ELSE}
...
//ADDED FOR LAZARUS
  {$IFDEF MWE_FPC}
    lfWeight := Bolds[fsBold in fStyle];
    lfItalic := Ord((fsItalic in fStyle));
    lfUnderline := Ord((fsUnderline in fStyle));
    lfStrikeOut := Ord((fsStrikeOut in fStyle));
  {$ELSE}
...
//ADDED FOR LAZARUS
  {$IFDEF MWE_FPC}
constructor TheTextDrawer.Create(CalcExtentBaseStyle: TFontStyles; _BaseFont: TFont);
begin
  inherited Create;

  FFontStock := TheFontStock.Create(_BaseFont);
  FCalcExtentBaseStyle := CalcExtentBaseStyle;
  SetBaseFont(_BaseFont);
  FColor := clWindowText;
  FBkColor := clWindow;
end;
  {$ELSE}
...
//ADDED FOR LAZARUS
  {$IFDEF MWE_FPC}
  LCLLinux.ExtTextOut(FDC, X, Y, fuOptions, @ARect, Text,Length, PInteger(FETODist));

  {$ELSE}
  Windows.ExtTextOut(FDC, X, Y, fuOptions, @ARect, Text,Length, PInteger(FETODist));
