...
begin
  ...
  // insert header
  fBuffer.SetSize(fBuffer.Position);                                            //ajb 2000-07-16
...
begin
  ...
  if AToken <> '' then begin
    ...
    while ISrc <= SrcLen do begin
      IsSpace := IsSpace and (AToken[ISrc] = ' ');
      if fReplaceReserved[AToken[ISrc]] <> nil then begin
        Replace := StrPas(fReplaceReserved[AToken[ISrc]]);
        Inc(ISrc);
{$IFDEF SYN_MBCSSUPPORT}
