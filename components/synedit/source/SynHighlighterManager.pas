...
uses
  DsgnIntf, Windows, SysUtils, Forms, Controls, StdCtrls, CheckLst,
  SynEditStrConst, SynEditHighlighter;
...
var
  form: TCustomForm;
  dsgn: {$IFDEF SYN_COMPILER_4_UP}IFormDesigner{$ELSE}TFormDesigner{$ENDIF};
  highlight: TSynHighlighterList;
...
begin
  ...
  if (csDesigning in ComponentState) and (AOwner is TCustomForm) then begin
    ...
    dsgn := form.Designer as {$IFDEF SYN_COMPILER_4_UP}IFormDesigner{$ELSE}TFormDesigner{$ENDIF};
