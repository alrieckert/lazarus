...
uses
  Classes, llEvents, llCommon,
  {$i sys/osunits.inc}

const
  { API compatibility, these had to be moved to prevent circular unit usage and a
    fpc bug with inline }
  LADDR_ANY   = llCommon.LADDR_ANY;
  LADDR_BR    = llCommon.LADDR_BR;
  LADDR_LO    = llCommon.LADDR_LO;
  LADDR6_ANY  = llCommon.LADDR6_ANY;
  LADDR6_LO   = llCommon.LADDR6_LO;
...
begin
  if FHandle >= 0 then // we already set our socket
    if not llCommon.SetBlocking(FHandle, aValue) then
